...
App.prototype = {
    ...
    route: function() {
        ...
        var renderer = this.renderer = new Renderer(this, this.api, this.apiCache, this.base);
    ...
    normalizeURL: function(URL, type) {
        return this.renderer.renderURL(URL, type, true);
    },
    ...
    _loadJS: function(modulePath, renderer, options, cb) {
        ...
        if (this.paths.length) {
            basePaths = [basePath];
            basePaths.push.apply(basePaths, this.paths);
        } else {
            basePaths = basePath;
        }

        pkg.searchScript(scriptPath, basePaths, abind(function(err, findings) {
            if (findings) {

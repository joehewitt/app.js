...
App.prototype = {
    ...
    route: function() {
        function handler(req, res, next) {
            ...
            this.loadURL(req.url, renderer, urlOptions, function(err, result) {
                if (err) {
    ...
    loadURL: function(URL, renderer, options, cb) {
        var u = url.parse(URL);
        var parts = u.pathname.split('/').slice(this.baseLength);
        var m = this.appjsPrefix.exec(parts[1]);
        if (m) {
            if (parts.length == 2) {
            ...
            this._loadPage(u, renderer, options, cb);
        }
    },

...
App.prototype = {
    get resourceBase() {
        return this.base + this.resourceName;
    },

    get resourceName() {
        return this.appVersion || this.resources;
    },
    ...
    get jsPath() {
        return this.resourceBase + "/app.js";
    },

    get cssPath() {
        return this.resourceBase + "/app.css";
    },
    ...
    get appjsPrefix() {
        if (!this.reResourcePrefix) {
            return this.reResourcePrefix = new RegExp("^" + this.resourceName + "(\\.\\d+)?(:.*?)?$");
        } else {
    ...
    route: function() {
        function handler(req, res, next) {
            ...
            urlOptions.userAgent = req.get('user-agent');

            if (this.allowOrigin) {
                res.set('Access-Control-Allow-Origin', this.allowOrigin);
            }
    ...
    _loadURL: function(urlParsed, parts, m, renderer, options, cb) {
        if (m) {
            if (parts.length == 2) {
                ...
                if (parts[2] == 'static') {
                    var staticPath = parts.slice(3).join('/');
                    staticPath = unescape(staticPath);
                    this._loadStatic(staticPath, renderer, options, cb);
                } else if (parts[2] == 'app.js') {
...
function uaware(app) {
    return function(req, res, next) {
        if (!req.appjsMatch && app.inlineContent) {
            var featureMap = app.featureDB.getFeatureMap(req.get('user-agent'), true);
            if (!featureMap) {
                forceContentInline();
            } else {

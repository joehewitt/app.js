...
exports.Renderer.prototype = {
    ...
    _scriptTags: function(urlPaths, options, cb) {
        ...
        if (options.loader == "dynamic") {
            ...
            tags.push(scriptTagForSource('window.appjsBase=' + JSON.stringify(baseURL)));
        }
    ...
    _urlForCDN: function(URL, category, isStatic) {
        var parsed = url.parse(URL);
        var timestamp = this._getLatestTimestamp(category, isStatic);
    ...
    renderURL: function(URL, category, isStatic) {
        if (this.app.cdn) {
            return this._urlForCDN(URL, category, isStatic);
        } else {
            return URL;
        }
    },

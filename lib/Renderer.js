...
exports.Renderer.prototype = {
        ...
        this._traceDependencies(modulePath, abind(function(err, dependencies) {
            ...
            async.map(features,
                _.bind(function(feature, cb2) {
                    var featureMap = options.userAgent ? this.featureDB.getFeatureMap(options.userAgent) : {};
                    if (!featureMap[feature]) {
                        var polyfillPath = path.join(__dirname, '..', 'client', 'polyfills', feature+'.js');
                        fs.lstat(polyfillPath, function(err, stat) {
                            if (!err && stat.isFile()) {
                                fs.readFile(polyfillPath, abind(function(err, data) {
                                     D&&D('found', polyfillPath, data+'');
                                     cb2(0, data+'');
                                }, cb, this))
                            } else {
                                cb2(0, '');
    ...
    renderScriptInline: function(scriptPath, modulePath, options, cb) {
        ...
        async.waterfall([
            ibind(function(next) {
                this._traceDependencies(this.app.clientModuleName, next);
            }, cb, this),
            ...
            ibind(function(dependencies, next) {
                options.featureMap = options.userAgent
                    ...
                    ? this.featureDB.getFeatureMap(options.userAgent)
    ...
    // relinkScript: function(modulePath, source, freeze, cb) {
    ...
    compressJavaScript: function(source, options) {
        try {
            ...
            var beautify = true;//'beautify' in options ? options.beautify : false;

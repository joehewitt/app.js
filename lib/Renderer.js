...
exports.Renderer.prototype = {
		...
		fs.stat(scriptPath, safeBind(cb, function(err, stat) {
			...
			function phase2(err, data) {
				if (options.inlineScripts && relink) {
					...
					this._traceDependencies(modulePath, safeBind(cb, phase3, this));
				} else {
					phase3.apply(this, [0]);
				}
			}
	...
	renderStylesheet: function(options, cb) {
		...
		if (options.inlineStyles) {
			this._traceDependencies(this.app.clientModuleName, safeBind(cb, phase2, this));
		} else {
			phase2.apply(this, [0, {}]);
		}
	...
	styleTags: function(options, cb) {
		...
		if (options.inlineStyles) {
			...
			this._traceDependencies(this.app.clientModuleName,
				safeBind(cb, function(err, results) {

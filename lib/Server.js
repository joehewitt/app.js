...
exports.listen = function(app, port, options) {
	...
	exports.route(server, app, null, null, options);
	server.listen(port || defaultPort);
	D&&D('Listening on port %d', server.address().port);
}

exports.congigure = function(server, options) {
};
...
exports.route = function(server, app, api, apiCache, options) {
	...
	var uaMiddleware = uaware(app, renderer);
	var cacheMiddleware = options.disableCache ? noop : cacheware(app.cache);
	server.get('/app.js*', cacheMiddleware, serveApp);
	server.get('*', uaMiddleware, cacheMiddleware, serveApp);
...
function uaware(app, renderer) {
	return function(req, res, next) {
		renderer.traceDependencies(app.clientModuleName, abind(function(err, deps) {
			var featureMap = renderer.featureDB.getFeatureMap(req.header('user-agent'), true);
			if (!featureMap) {
				forceContentInline();
			} else {
				for (var name in featureMap) {
					var isRequired = deps.has[name];
					// D&&D(name, 'req', isRequired, 'support', featureMap[name], 'poly', renderer.polyfills[name]);
					if (isRequired && !featureMap[name] && !renderer.polyfills[name]) {
						forceContentInline();
						break;
					}
				}

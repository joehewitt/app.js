...
Loader.prototype = {
    loadURL: function(URL, renderer, options, cb) {
        ...
        var m = reAppJSPrefix.exec(parts[1]);
        if (m) {
            if (parts.length == 2) {
                this.loadAppjs(renderer, options, cb);
            } else {
                if (parts[2] == 'static') {
                    this.loadStatic(parts.slice(3).join('/'), renderer, options, cb);
                } else if (parts[2] == 'js') {
                    ...
                    this.loadHas(parts.slice(3).join('/'), renderer, options, cb);
                } else if (parts[2] == 'polyfills') {
                    this.loadPolyfills(parts.slice(3).join('/'), renderer, options, cb);
                } else {
                    cb("Not found");
                }
            }
        ...
        } else if (!renderer.app.settings.autohtml) {
            var subpath = parts.slice(1).join('/');
            if (!subpath) {
                subpath = 'index.html';
            }
            var newURL = '/app.js/static/' + renderer.app.packageInfo.name + '/' + subpath;
            this.loadURL(newURL, renderer, options, cb);
        } else {
            this.loadPage(u, renderer, options, cb);
        }
    },
    ...
    loadAppjs: function(renderer, options, cb) {
        var scriptName = options.js == "inline" ? "appjs-inline.js" : "appjs.js";
    ...
    loadStatic: function(staticPath, renderer, options, cb) {
    ...
    loadJS: function(modulePath, renderer, options, cb) {
        var scriptPath = this._cleanJSPath(modulePath, renderer);
        var basePath = scriptPath == renderer.app.clientModuleName ? null : renderer.app.clientModuleName;
        this.searchScript(scriptPath, basePath, abind(function(err, findings) {
            if (findings) {
    ...
    loadHas: function(basePath, renderer, options, cb) {
        var scriptPath = this._cleanJSPath(basePath, renderer);
        this.searchScript(scriptPath, abind(function(err, findings) {
            if (findings) {
                renderer.renderHas(findings.path, findings.name, options,
                    abind(function(err, result) {
    ...
    loadPolyfills: function(basePath, renderer, options, cb) {
        var scriptPath = this._cleanJSPath(basePath, renderer);
        this.searchScript(scriptPath, abind(function(err, findings) {
            if (findings) {
                renderer.renderPolyfills(findings.path, findings.name, options,
                    abind(function(err, result) {
                        cb(0, {path: result.path, body: result.source, dependencies: result.dependencies});
                    }, cb)
                );
    ...
    searchScript: function(modulePath, basePath, cb) {
        ...
        if (modulePath[0] == '/') {
            complete.apply(this, [modulePath]);
        } else {
            ...
            if (baseScriptPath && modulePath[0] == '.') {
                ...
                complete.apply(this, [absolutePath]);
            } else if (baseScriptPath) {
                var baseDir = path.dirname(baseScriptPath);
                findInNodeModules.apply(this, [baseDir]);
            } else {
                complete.apply(this, [modulePath]);
            }
        }
            ...
            fs.lstat(containerPath, abind(function(err, stat) {
                if (stat.isDirectory()) {
                    ...
                    try {
                        ...
                        complete.apply(this, [foundPath]);                        
                    } catch (exc) {
                        if (containerPath != '/') {
                            findInNodeModules.apply(this, [path.dirname(containerPath)]);
                        } else {
                            fail();
                        }
                    }
        ...
        function complete(modulePath) {
            ...
            this.shortenModulePath(scriptPath, abind(function(err, shortModulePath) {
    ...
    searchStatic: function(modulePath, basePath, isURL, cb) {
        ...
        try {
            ...
            var absolutePath = this.normalizeName(modulePath, basePath, isURL);
                    ...
                    fs.lstat(findings.path, tbind(function(err, stat) {
                        fs.readFile(findings.path, tbind(function(err, source) {
                            ...
                            while (m = reStyle.exec(source)) {
                                var stylesheetPath = this.normalizeName(m[1], findings.name);
    ...
    traceStylesheet: function(modulePath, recursive, source, sourceLine, cb) {
        ...
        function scanSource(source, basePath) {
            ...
            while ((m = reCSSImport.exec(source)) && m[2]) {
                var stylesheetPath = this.normalizeName(m[2], basePath, true);
                traceStylesheet.apply(this, [stylesheetPath, m[1], m[0]]);
            }

            while ((m = reCSSURL.exec(source)) && m[1]) {
                var imagePath = this.normalizeName(m[1], basePath, true);
    ...
    normalizeName: function(name, baseName, isURL) {
        if (isURL) {

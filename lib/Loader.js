...
Loader.prototype = {
	loadURL: function(url, pageMaker, options, cb) {
		...
		if (parts[1] == '') {
			...
			this.loadFile(appjsPath, '', options, false, false/*use cache*/,
			...
			this.searchScript(relativeURL, safeBind(cb, function(err, findings) {
				if (findings) {
					this.loadFile(findings.path, findings.name, options, true, false/*use cache*/,
						safeBind(cb, function(err, source) {
	...
	loadFile: function(scriptPath, modulePath, options, relink, useCache, cb) {

...
function Loader(paths) {
	...
	require.paths.push.apply(require.paths, paths);
	this.cache = {};
	this.apps = {};
...
Loader.prototype = {
	...
	load: function(url, compress, munge, cb) {
		...
		if (parts[1] == 'static') {
			...
			var relativeURL = parts.slice(2).join('/');
			this.searchScript(relativeURL, function(err, findings) {
				if (findings) {
					this.loadSource(findings.path, compress, munge, compress, function(err, source) {
	...
	loadSource: function(searchPath, compress, munge, useCache, cb) {
		if (munge === undefined) munge = true;
		
		fs.stat(searchPath, _.bind(function(err, stat) {
			if (err) { cb(err); return; }

			if (useCache) {
				var cached = this.cache[searchPath];
				if (cached.sourceTime <= stat.mtime) {
					cb(0, cached.source);
					return;
				}
			}

			fs.readFile(searchPath, _.bind(function(err, data) {
				if (err) { cb(err); return; }

				if (compress) {
					data = this._compress(data);
					this.cache[searchPath] = {source: compressed, sourceTime: stat.mtime};
				}
	...
	traceDependencies: function(rootModuleName, skipDebug, cb) {
		...
		// XXXjoe Parse require() instead of define()

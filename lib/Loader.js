...
Loader.prototype = {
	...
	traceDependencies: function(rootModuleName, skipDebug, cb) {
		...
		function traceScript(modulePath, basePath, sourceLine) {
			...
			this.searchScript(modulePath, basePath, _.bind(function(err, findings) {
				if (err || !findings) { if (!--depth) cb(0, results); return; }

				if (modulePath in moduleMap) {
					complete();
				} else {
					fs.lstat(findings.path, _.bind(function(err, stat) {
						if (err) { if (!--depth) cb(0, results); return; }

						fs.readFile(findings.path, _.bind(function(err, source) {
							if (err) { if (!--depth) cb(0, results); return; }
		...
		function traceStylesheet(stylesheetPath, sourceLine) {
			if (!(stylesheetPath in styleMap)) {
				...
				this.traceStylesheet(stylesheetPath, true, null, sourceLine, _.bind(function(err, res) {
		...
		function traceIcons(rootModuleName) {
			++depth;

			this.findPackageStaticPath(rootModuleName, _.bind(function(err, staticPath) {
				fs.readdir(staticPath, abind(function(err, paths) {
